 ...
 * @author jamesdbloom
 */
public class NettyReverseProxyIntegrationTest {

    private final static Logger logger = LoggerFactory.getLogger(NettyReverseProxyIntegrationTest.class);

    private final static Integer SERVER_HTTP_PORT = PortFactory.findFreePort();
    private final static Integer PROXY_LOCAL_PORT = PortFactory.findFreePort();
    ...
    public static void setupFixture() throws Exception {
        ...
        logger.debug("PROXY_LOCAL_PORT = " + PROXY_LOCAL_PORT);
        ...
        proxy = new ProxyBuilder()
                .withLocalPort(PROXY_LOCAL_PORT)
                .withRemote("127.0.0.1", SERVER_HTTP_PORT)
                .build();
    }
    ...
    public void shouldForwardRequestsUsingSocketDirectlyHeadersOnly() throws Exception {
        Socket socket = null;
        try {
            socket = new Socket("localhost", PROXY_LOCAL_PORT);
